        ...
        try:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    status = 'WARN'
                    message = 'Could not verify BAM with samtools'
                    total_reads = None
                    is_paired = None
            
            except FileNotFoundError:
                logger.warning("samtools not found, performing basic validation")
                status = 'WARN'
                message = 'BAM magic number valid, but samtools unavailable for full validation'
                total_reads = None
                is_paired = None
            ...
            with open(file_path, 'r') as f:
                for line_num, line in enumerate(f):
                    ...
                    if line_num == 0:
                        ...
                        for col_idx in range(1, min(len(fields), col_count)):
                            try:
                                val = float(fields[col_idx])
                                if val < 0:
                                    if row_count <= 5:
